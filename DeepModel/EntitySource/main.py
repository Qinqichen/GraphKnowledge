...
def evaluate(data, model, name, nbest=None):
    ...
    #  qqc + 1 确保除数不为零
    speed = len(instances)/(decode_time + 1)
...
def load_model_decode(data, name):
    ...
    # data.load_model_dir = data.model_dir+'.'+ str(data.nbest) + '.model'
    data.load_model_dir = data.model_dir+'.'+ str(0) + '.model'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_dir', default=base_path+"sample_data/train.char.bmes")
    parser.add_argument('--dev_dir', default=base_path+"sample_data/dev.char.bmes" )
    parser.add_argument('--test_dir', default=base_path+"sample_data/test.char.bmes")

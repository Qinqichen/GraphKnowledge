...
class Data:
    ...
    def show_data_summary(self):
        ...
        print("     Model        use_position: %s" % (self.position_emb))
        print("     Model        use_fasttext: %s" % (self.fasttext_emb))
        print("     Model        use_lm_context: %s" % (self.use_lm_context))
        print("     Model        use_att: %s" % (self.use_att))
        print("     Model        use_stack: %s" % (self.use_stack))
        print("     Model        use_linear: %s" % (self.use_linear))
        print("     Model        use_word_dropout: %s" % (self.use_word_dropout))
        print("     Model        emb_grad: %s" % (self.emb_grad))
        ...
        print("     Hyper         HP_clip: %s"%(self.whether_clip_grad))
        ...
        print("     Hyper      stack_layer: %s"%(self.stack_layer))
        print("     Hyper      num_attention_head: %s"%(self.num_attention_head))
        print("     Hyper      doc_kernel_size: %s"%(self.doc_kernel_size))
        print("     Hyper      doc_dim: %s"%(self.doc_dim))
        print("     Hyper      doc_depth: %s"%(self.doc_depth))
    ...
    def build_pretrain_emb(self):
        if self.position_emb:
            ...
            for i in range(self.max_scent_len):
                sent_pos = get_timing_signal_1d(i+1,self.pos_emb_dim,start_index=1)
                if self.pretrain_position_embedding is None:
                    self.pretrain_position_embedding = sent_pos
                else:
                    self.pretrain_position_embedding = torch.cat([self.pretrain_position_embedding,sent_pos],0)
        ...
        if self.word_emb_dir:
            ...
            if self.fasttext_emb:
                self.pretrain_fasttext_embedding, self.fasttext_emb_dim = build_pretrain_gensim_embedding(self.fasttext_emb_dir,
                                                                                           self.word_alphabet,
                                                                                           100,
                                                                                           self.norm_word_emb)
    ...
    def generate_instance(self, name):
        ...
        if name == "train":
            ...
            self.dev_texts, self.dev_Ids,self.max_scent_len = read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token,char_padding_size=-1, char_padding_symbol = '</pad>',max_sent_len=self.max_scent_len)
        elif name == "test":
            self.test_texts, self.test_Ids,self.max_scent_len = read_instance(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token,char_padding_size=-1, char_padding_symbol = '</pad>',max_sent_len=self.max_scent_len)
        elif name == "raw":
    ...
    def read_config_args(self, args):
        ...
        self.fasttext_emb_dir = args.fasttext_emb_dir

        self.norm_word_emb = args.norm_word_emb

        self.norm_char_emb = args.norm_char_emb

        self.number_normalized = args.number_normalized

        self.position_emb = args.position_emb

        self.fasttext_emb = args.fasttext_emb
        ...
        self.pos_emb_dim = int(args.pos_emb_dim)
        #new
        self.lm_context_dim = int(args.lm_context_dim)
        self.use_lm_context = args.use_lm_context
        self.lm_dir = args.lm_dir
        self.use_crf = args.use_crf
        self.use_linear = args.use_linear
        self.use_char = args.use_char
        self.use_att =args.use_att
        self.use_stack = args.use_stack
        self.stack_layer = int(args.stack_layer)
        self.use_word_dropout = args.use_word_dropout
        self.bad_epochs_patience = args.bad_epochs_patience
        self.min_lr = args.min_lr
        self.show_batchs = args.show_batchs if int(args.show_batchs) != 0 else args.batch_size * 50
        self.word_feature_extractor = args.word_seq_feature
        self.emb_grad = args.emb_grad
        self.char_feature_extractor = args.char_seq_feature
        self.doc_kernel_size = int(args.doc_kernel_size)
        self.doc_dim = int(args.doc_dim)
        self.doc_depth = int(args.doc_depth)
        ...
        self.label_emb_scale = float(args.label_emb_scale)

        self.num_attention_head = int(args.num_attention_head)

        self.whether_clip_grad = args.whether_clip_grad
    ...
    def read_config(self,config_file):
        ...
        the_item = 'label_emb_scale'
        if the_item in config:
            self.label_emb_scale = int(config[the_item])
        ## read network:
        the_item = 'use_crf'
        if the_item in config:
